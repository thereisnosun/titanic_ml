...
def get_feature_matrix(dataset):
    ...
    print(list(dataset))
    feature_matrix = dataset.as_matrix(columns=['Pclass', 'Sex', 'ActualFare', 'Age', 'FamilySize',
...
def hyper_param_search(feature_matrix, feature_output):
    ...
    forest_reg = RandomForestClassifier(random_state=42,  n_estimators=50);
...
def main():
    ...
    final_model = RandomForestClassifier(random_state=42, n_estimators=100,oob_score=True,max_features=5)
